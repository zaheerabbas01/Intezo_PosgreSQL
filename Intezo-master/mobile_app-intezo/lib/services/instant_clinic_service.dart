...
class InstantClinicService {
  ...
  static Future<Map<String, dynamic>> loadClinicDataFast(
    String clinicId,
  ) async {
    ...
    try {
      ...
      final doctorsResponse = await ApiService.get(
      ...
      if (doctorsResponse is List && doctorsResponse.isNotEmpty) {
        ...
        final queueFutures = doctors
            .map(
              (doctor) =>
                  ApiService.get(
                        'queues/public/$clinicId/${doctor['id']}',
                        isPublic: true,
                      )
                      .then(
                        (queueData) => {
        ...
        final doctorsWithQueues = doctors.map((doctor) {
          final queueResult = queueResults.firstWhere(
            (result) => result['doctorId'] == doctor['id'],
            orElse: () => {
              'doctorId': doctor['id'],
              'queueData': {
                'current': 0,
          ...
          return {...doctor, 'queueInfo': queueResult['queueData']};
        }).toList();

        stopwatch.stop();
        print(
          '⚡ INSTANT: Loaded ${doctors.length} doctors with queues in ${stopwatch.elapsedMilliseconds}ms',
        );
  ...
  static Future<Map<String, dynamic>> _loadSequential(String clinicId) async {
    ...
    try {
      ...
      stopwatch.stop();
      print(
        '🐌 SEQUENTIAL: Loaded ${doctors.length} doctors in ${stopwatch.elapsedMilliseconds}ms',
      );
