...
class PreloadService {
    ...
    try {
      ...
      if ((now - cacheTime) < (10 * 60 * 1000)) {
        ...
        if (cachedClinics.isNotEmpty) {
          final topClinicIds = cachedClinics
              .take(3)
  ...
  static Future<void> _fetchRecentClinicsBackground(
    String patientId,
    ...
    try {
      ...
      if (response != null && response is List) {
        ...
        final sortedBookings = List<dynamic>.from(response)
          ..sort((a, b) {
            final aDate =
                DateTime.tryParse(a['servedAt'] ?? a['bookedAt'] ?? '') ??
                DateTime(1970);
            final bDate =
                DateTime.tryParse(b['servedAt'] ?? b['bookedAt'] ?? '') ??
                DateTime(1970);
        ...
        for (final booking in sortedBookings) {
          final bookingDate = DateTime.tryParse(
            booking['servedAt'] ?? booking['bookedAt'] ?? '',
          );
          final clinicData = booking['clinic'];

          if (bookingDate != null &&
              bookingDate.isAfter(threeMonthsAgo) &&
              clinicData != null &&
              !seenIds.contains(clinicData['id'])) {
            seenIds.add(clinicData['id']);
        ...
        if (recentClinics.isNotEmpty) {
          final topClinicIds = recentClinics
              .take(3)
              .map((c) => c['id']?.toString())
              .where((id) => id != null)
              .cast<String>()
  ...
  static Future<List<Map<String, dynamic>>> getCachedRecentClinics(
    String patientId,
  ) async {
    try {
      final prefs = await SharedPreferences.getInstance();
  ...
  static Future<void> batchPreloadClinics(List<String> clinicIds) async {
    try {
      ...
      for (final batch in batches) {
        final preloadPromises = batch.map(
          (clinicId) => OptimizedClinicService.preloadClinicData(clinicId),
        );
      ...
      print(
        'PreloadService: Batch preload completed for ${clinicIds.length} clinics',
      );
    } catch (e) {
  ...
  static Future<Map<String, dynamic>?> getInstantClinicData(
    String clinicId,
  ) async {
    try {
    ...
    try {
      ...
      if (recentClinics.isNotEmpty) {
        final topClinicIds = recentClinics
            ...
            .cast<String>()
            .toList();
        await batchPreloadClinics(topClinicIds);
      }
