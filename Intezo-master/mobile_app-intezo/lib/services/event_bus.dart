...
class EventBus {
  ...
  final _clinicStatusController =
      StreamController<ClinicStatusUpdateEvent>.broadcast();
  final _doctorAvailabilityController =
      StreamController<DoctorAvailabilityUpdateEvent>.broadcast();
  final _patientUpdateController =
      StreamController<PatientUpdateEvent>.broadcast();
  final _patientServedController =
      StreamController<PatientServedEvent>.broadcast();
  final _systemNotificationController =
      StreamController<SystemNotificationEvent>.broadcast();
  final _bookingHistoryController =
      StreamController<BookingHistoryUpdateEvent>.broadcast();

  Stream<QueueUpdateEvent> get onQueueUpdate => _queueUpdateController.stream;
  Stream<ClinicStatusUpdateEvent> get onClinicStatusUpdate =>
      _clinicStatusController.stream;
  Stream<DoctorAvailabilityUpdateEvent> get onDoctorAvailabilityUpdate =>
      _doctorAvailabilityController.stream;
  Stream<PatientUpdateEvent> get onPatientUpdate =>
      _patientUpdateController.stream;
  Stream<PatientServedEvent> get onPatientServed =>
      _patientServedController.stream;
  Stream<SystemNotificationEvent> get onSystemNotification =>
      _systemNotificationController.stream;
  Stream<BookingHistoryUpdateEvent> get onBookingHistoryUpdate =>
      _bookingHistoryController.stream;
