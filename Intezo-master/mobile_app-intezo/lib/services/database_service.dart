...
class DatabaseService {
  ...
  static Future<Database> _initDatabase() async {
    ...
    final password = await SecureStorageService.getOrCreateDatabaseKey();
    ...
    // Cached data can always be fetched again. Remove the previous unencrypted
    // database after the encrypted database opens successfully.
  ...
  static Future<void> saveBookings(List<Booking> bookings) async {
    ...
    if (bookings.isNotEmpty) {
      await db.delete(
        'bookings',
        where: 'patientId = ?',
        whereArgs: [bookings.first.patientId],
      );
    }
    ...
    await batch.commit();
    if (kDebugMode) {
      debugPrint('DatabaseService: Cached ${bookings.length} bookings');
    }
  }
  ...
  static Future<List<Booking>> getBookingHistory(String patientId) async {
    ...
    if (kDebugMode) {
      debugPrint('DatabaseService: Loaded ${maps.length} cached bookings');
    }
  ...
  static Future<void> saveEmailToHistory(String email) async {
    ...
    if (existing.isNotEmpty) {
      ...
      await db.update(
        'email_history',
        {'lastUsed': now, 'useCount': (existing.first['useCount'] as int) + 1},
        where: 'email = ?',
        whereArgs: [email],
      );
    } else {
      // Insert new entry
      await db.insert('email_history', {
        'email': email,
        'lastUsed': now,
        'useCount': 1,
      });
