...
class SocketService {
  ...
  Future<void> connect({String? clinicId, String? doctorId}) async {
    print(
      'SocketService.connect called for clinic: $clinicId, doctor: $doctorId',
    );
    ...
    try {
      ...
      // Import ApiConfig to use the configured URL
      final serverUrl = ApiConfig.socketBaseUrl;
  ...
  void _handleSocketEvent(String eventName, dynamic data) {
    print(
      'Processing Socket.IO event: $eventName for clinic: $_currentClinicId',
    );
    ...
    try {
      ...
      if (eventName == 'queue_updated' || eventName == 'wait_time_updated') {
        ...
        EventBus().emitClinicStatusUpdate(
          ClinicStatusUpdateEvent(
            clinicId: clinicId,
            statusData: {
              'isOpen': data['isOpen'],
  ...
  Future<void> joinClinicRoom(String clinicId, {String? doctorId}) async {
    ...
    try {
      ...
      print(
        'Successfully joined all rooms for clinic: $clinicId, doctor: $doctorId',
      );
    } catch (e) {
  ...
  bool get isActive {
    ...
    print(
      'Socket.IO isActive check: connected=$isConnected, clinic=$_currentClinicId, active=$active',
    );
    return active;
  }
}
