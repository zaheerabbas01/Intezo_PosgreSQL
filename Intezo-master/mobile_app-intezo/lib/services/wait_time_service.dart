...
class WaitTimeService {
  ...
  static Future<Map<String, dynamic>?> getWaitTime(
    String clinicId,
    String doctorId,
  ) async {
    try {
  ...
  static Future<Map<String, dynamic>?> getPatientWaitTime(
    String queueId,
  ) async {
    try {
      final response = await ApiService.get(
  ...
  static Future<Map<String, dynamic>?> getDetailedQueue(
    String clinicId,
    String doctorId,
  ) async {
    try {
  ...
  static Map<String, dynamic> calculateWaitTime({
    ...
    final estimatedMinutes = patientsAhead > 0
        ? patientsAhead * avgProcessTime
        : 0;
