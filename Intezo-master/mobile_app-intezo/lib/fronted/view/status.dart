...
class _StatusState extends State<Status> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Queue Status", style: TextStyle(color: context.textColor)),
          ...
          IconButton(
            ...
            onPressed: () async {
              ...
              if (_queueData != null && _queueData!['currentQueue'] != null) {
                ...
                final cq = _queueData!['currentQueue'];
                final clinicId =
                    (cq['clinic']?['id'] ?? cq['clinic']?['_id']) as String?;
                final doctorId =
                    (cq['doctor']?['id'] ?? cq['doctor']?['_id']) as String?;
                if (clinicId != null) {
  ...
  Widget _buildMainContent(bool isDarkMode) {
    ...
    if (_queueData != null &&
        _queueData!['isPremium'] == true &&
        _queueData!['activeBookings'] != null) {
  ...
  void _setupRealTimeUpdates() {
    _queueUpdateSubscription = EventBus().onQueueUpdate.listen((event) {
      ...
      // Check if patient's number is being served, skipped, or passed
      final currentNumber = event.queueData['currentNumber'];
      if (_queueData != null && _queueData!['currentQueue'] != null) {
        final patientNumber = _queueData!['currentQueue']['number'];
        final currentStatus = _queueData!['currentQueue']['status'];

        // Check if patient was skipped (currentNumber passed patient number without being served)
        // OR patient's number is being called OR patient is already served
        final wasSkipped =
            currentNumber > patientNumber &&
            currentStatus != 'served' &&
            currentStatus != 'skipped';
        final isServed =
            currentNumber == patientNumber && currentStatus == 'served';

        if (currentNumber == patientNumber ||
            wasSkipped ||
            isServed ||
            event.queueData['skippedPatients']?.contains(patientNumber) ==
                true) {
          print(
            'Patient number $patientNumber: currentNumber=$currentNumber, status=$currentStatus - reloading!',
          );
          // Patient is being served or skipped - reload status to check
        ...
        if (mounted) {
          setState(() {
            ...
            final serverPosition =
                _queueData!['currentQueue']['positionInQueue'];
            final positionInQueue =
                serverPosition ?? (patientNumber - currentNumber);
            final avgProcessTime =
                event.queueData['avgProcessTimeMinutes'] ??
                event.queueData['avgWaitTime'] ??
                15;
            final estimatedWait = positionInQueue > 0
                ? positionInQueue * avgProcessTime
                : 0;

            _queueData!['currentQueue']['currentServing'] = currentNumber;
            _queueData!['currentQueue']['positionInQueue'] = positionInQueue > 0
                ? positionInQueue
                : 0;
            _queueData!['currentQueue']['estimatedWait'] = estimatedWait;
            _queueData!['currentQueue']['estimatedWaitTime'] =
                WaitTimeService.formatWaitTime(estimatedWait);
            _queueData!['currentQueue']['estimatedWaitMinutes'] = estimatedWait;
            _queueData!['currentQueue']['patientsAhead'] = positionInQueue > 0
                ? positionInQueue
                : 0;
        ...
        if (_queueData != null &&
          ...
          for (int i = 0; i < activeBookings.length; i++) {
            ...
            final bookingClinicId = booking['clinic']?['id'];
            final bookingDoctorId = booking['doctor']?['id'];
              ...
              if (patientNumber != null) {
                ...
                if (mounted) {
                  setState(() {
                    ...
                    activeBookings[i]['positionInQueue'] = positionInQueue > 0
                        ? positionInQueue
                        : 0;
                    activeBookings[i]['estimatedWaitMinutes'] = estimatedWait;
                    activeBookings[i]['estimatedWaitTime'] =
                        WaitTimeService.formatWaitTime(estimatedWait);
                    activeBookings[i]['patientsAhead'] = positionInQueue > 0
                        ? positionInQueue
                        : 0;
    ...
    _patientServedSubscription = EventBus().onPatientServed.listen((event) {
      ...
      if (_queueData != null && _queueData!['currentQueue'] != null) {
        final currentQueueId = _queueData!['currentQueue']['id'];
  ...
  Future<void> _loadCurrentQueueStatus() async {
    ...
    try {
      ...
      // No data or explicit no-booking response
      if (queueData == null ||
      ...
      // Premium: multiple active bookings
      if (queueData['isPremium'] == true &&
          queueData['activeBookings'] != null) {
        final bookings = queueData['activeBookings'] as List;
        ...
        for (var booking in bookings) {
          ...
          final clinicId =
              (booking['clinic']?['id'] ?? booking['clinic']?['_id'])
                  as String?;
          final doctorId =
              (booking['doctor']?['id'] ?? booking['doctor']?['_id'])
                  as String?;
          if (clinicId != null) {
            clinicProvider.startListeningForUpdates(
              clinicId,
      ...
      // Non-premium: single booking
      final currentQueue = queueData['currentQueue'];
      ...
      final status = currentQueue['status'] as String?;
      ...
      if (status == 'skipped') {
        // Keep the queue data but display skipped message
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Your turn was skipped. Please contact the clinic.'),
      ...
      // backend returns 'id', not '_id'
      final clinicId =
          (currentQueue['clinic']?['id'] ?? currentQueue['clinic']?['_id'])
              as String?;
      final doctorId =
          (currentQueue['doctor']?['id'] ?? currentQueue['doctor']?['_id'])
              as String?;
      if (clinicId != null) {
  ...
  Future<void> _updateWaitTimes() async {
    ...
    try {
      ...
      // Handle single booking - only update if real-time data seems stale
        ...
        final queueId = _queueData!['currentQueue']['id'];
        if (queueId != null) {
          final waitTime = await WaitTimeService.getPatientWaitTime(queueId);
          if (waitTime != null && mounted) {
            // Only update if the server data is different from current data
            final currentServing =
                _queueData!['currentQueue']['currentServing'] ?? 0;
            final serverCurrentServing = waitTime['currentlyServing'] ?? 0;

            if (serverCurrentServing != currentServing) {
              setState(() {
                _queueData!['currentQueue']['estimatedWaitMinutes'] =
                    waitTime['estimatedWaitMinutes'];
                _queueData!['currentQueue']['estimatedWaitTime'] =
                    waitTime['estimatedWaitTime'];
                _queueData!['currentQueue']['patientsAhead'] =
                    waitTime['patientsAhead'];
                _queueData!['currentQueue']['currentlyServing'] =
                    waitTime['currentlyServing'];
                _queueData!['currentQueue']['positionInQueue'] =
                    waitTime['patientsAhead'];
                _queueData!['currentQueue']['estimatedWait'] =
                    waitTime['estimatedWaitMinutes'];
              });
            }
          }
  ...
  Widget _buildPremiumBookingsList(List activeBookings, bool isDarkMode) {
    // Filter out served bookings, keep active and skipped bookings
    final filteredBookings = activeBookings
        .where((booking) => booking['status'] != 'served')
        .toList();

    if (filteredBookings.isEmpty) {
      return _buildNoQueueEncouragement(isDarkMode);
    }
      ...
      child: Column(
        ...
        children: [
          ...
          ...filteredBookings.map(
            (booking) => _buildBookingListItem(booking, isDarkMode),
          ),
        ],
      ),
    );
  ...
  Widget _buildBookingListItem(dynamic booking, bool isDarkMode) {
    ...
    final isBeingServed = positionInQueue <= 0 && !isSkipped;
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: isSkipped
                        ? Colors.orange
                        : (isBeingServed ? Colors.green : colors().bluecolor1),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (!isBeingServed &&
                          !isSkipped &&
                          booking['estimatedWaitTime'] != null)
                        Column(
                          children: [
                            const SizedBox(height: 2),
                            Text(
  ...
  Widget _buildCurrentQueueCard(dynamic queueData, bool isDarkMode) {
    ...
    final queueNumber =
        queueData['number'] ?? queueData['queueNumber'] ?? 'N/A';
    ...
    // Check if patient was skipped
    final isSkipped = status == 'skipped';

    // Show if patient is currently being served
    final isBeingServed = positionInQueue <= 0 && !isSkipped;
    // backend returns 'queueId' for the queue identifier
    final queueId = (queueData['queueId'] ?? queueData['id']) as String?;
          ...
          children: [
            ...
            if (isSkipped)
              ...
              _buildStatusRow(
                ...
                isBeingServed ? Colors.green.shade700 : Colors.orange.shade700,
                isDarkMode,
              ),
            if (isSkipped)
              _buildStatusRow(
            ...
            if (queueData['estimatedWaitMinutes'] != null &&
                !isBeingServed &&
                !isSkipped)
              _buildStatusRow(
                'Patients Ahead:',
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'Your turn was skipped by the doctor. Please contact the clinic for assistance.',
                        style: TextStyle(
                          color: Colors.orange.shade700,
                          fontWeight: FontWeight.w600,
                          fontSize: 13,
              ...
              children: [
                  ...
                  child: ElevatedButton(
                    onPressed: () async {
                      ...
                      if (_queueData != null &&
                        ...
                        final cq = _queueData!['currentQueue'];
                        final clinicId =
                            (cq['clinic']?['id'] ?? cq['clinic']?['_id'])
                                as String?;
                        final doctorId =
                            (cq['doctor']?['id'] ?? cq['doctor']?['_id'])
                                as String?;
                        if (clinicId != null) {
                          clinicProvider.startListeningForUpdates(
                            clinicId,
                ...
                Expanded(
                  child: OutlinedButton(
                    onPressed: queueId != null
                        ? () => _cancelBooking(queueId)
                        : null,
        ...
        return AlertDialog(
          ...
          actions: [
            TextButton(
              ...
              child: Text("No", style: TextStyle(color: context.subtextColor)),
            ),
            TextButton(
