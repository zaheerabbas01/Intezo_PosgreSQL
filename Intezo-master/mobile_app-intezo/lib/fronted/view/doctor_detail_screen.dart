    ...
    try {
              ...
              .map(
                (c) => {
                  'clinic': {
                    ...
                    'profilePhoto':
                        c['profilePhoto'], // Include clinic profile photo
                    'isOpen': c['isOpen'],
                  },
                  'fee': c['fee'],
                  'timing': c['timing'],
      ...
      for (var clinic in clinicProvider.clinics) {
        try {
          ...
          final doctorExists = doctors.any(
            (d) => d['_id'] == widget.doctor['id'],
          );
          if (doctorExists) {
            final doctorInClinic = doctors.firstWhere(
              (d) => d['_id'] == widget.doctor['id'],
            );
            ...
            doctorClinics.add({
              ...
              'timing':
                  doctorInClinic['availableHours'] ??
                  doctorInClinic['timing'] ??
                  'Not specified',
              'isAvailable': (clinic['isOpen'] ?? true)
                  ? (doctorInClinic['isAvailable'] ?? false)
                  : false,
          ...
          children: [
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    Container(
                      ...
                      child: ClipRRect(
                        ...
                        child: widget.doctor['profilePhoto'] != null
                            ? Image.network(
                                widget.doctor['profilePhoto'].startsWith(
                                      'https://',
                                    )
                    ...
                    const SizedBox(height: 16),
                    FutureBuilder<bool>(
                      future: NotificationService().isDoctorNotificationEnabled(
                        widget.doctor['id'],
                      ),
                      builder: (context, snapshot) {
                        final isEnabled = snapshot.data ?? false;
                        final isAvailable =
                            widget.doctor['isAvailable'] ?? true;
                        return SizedBox(
                          width: double.infinity,
            ...
            if (_isLoading)
              ...
              Container(
                ...
                child: Column(
                  children: [
                    Icon(Icons.business, size: 48, color: context.subtextColor),
                    const SizedBox(height: 12),
                    Text(
                      'No clinics found',
                      style: TextStyle(
              ...
              ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final isClinicAvailable =
                      clinicData['isAvailable'] && clinicData['isActive'];
