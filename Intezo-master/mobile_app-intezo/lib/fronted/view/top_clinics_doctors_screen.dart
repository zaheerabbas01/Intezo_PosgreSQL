...
class TopClinicsDoctorsScreen extends StatefulWidget {
  ...
  @override
  State<TopClinicsDoctorsScreen> createState() =>
      _TopClinicsDoctorsScreenState();
}

class _TopClinicsDoctorsScreenState extends State<TopClinicsDoctorsScreen>
  ...
  void _updateClinicStatus(String clinicId, Map<String, dynamic> statusData) {
    setState(() {
      ...
      final filteredIndex = _filteredClinics.indexWhere(
        (c) => c['id'] == clinicId,
      );
      if (filteredIndex != -1) {
        _filteredClinics[filteredIndex]['isOpen'] =
  ...
  void _updateDoctorAvailability(String doctorId, bool isAvailable) {
    setState(() {
      ...
      if (doctorIndex != -1) {
        ...
        final clinics =
            _doctors[doctorIndex]['clinics'] as List<Map<String, dynamic>>? ??
            [];
      ...
      final filteredIndex = _filteredDoctors.indexWhere(
        (d) => d['id'] == doctorId,
      );
      if (filteredIndex != -1) {
        _filteredDoctors[filteredIndex]['isAvailable'] = isAvailable;
        final clinics =
            _filteredDoctors[filteredIndex]['clinics']
  ...
  Future<void> _loadDoctors() async {
    ...
    for (var clinic in _clinics) {
      try {
        final doctors = await clinicProvider.getDoctors(clinic['id']);
        for (var doctor in doctors) {
          ...
          (uniqueDoctors[doctorId]!['clinics'] as List<Map<String, dynamic>>)
              .add({
                'clinicId': clinic['id'],
                'clinicName': clinic['name'],
                'clinicAddress': clinic['address'],
                'profilePhoto':
                    clinic['profilePhoto'], // Preserve clinic profile photo
        ...
        _filteredDoctors = _doctors.where((doctor) {
          return doctor['name']?.toLowerCase().contains(query.toLowerCase()) ==
              ...
              doctor['specialty']?.toLowerCase().contains(
                  ...
                  true ||
              doctor['clinicName']?.toLowerCase().contains(
                    query.toLowerCase(),
                  ) ==
                  true;
        }).toList();
        ...
        children: [
          ...
          Expanded(
            child: _isLoading
                ...
                : TabBarView(
                    controller: _tabController,
                    children: [_buildClinicsTab(), _buildDoctorsTab()],
                  ),
          ),
        ],
      ),
    ...
    if (_filteredClinics.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            Icon(Icons.business, size: 64, color: context.subtextColor),
            const SizedBox(height: 16),
            Text(
              _searchQuery.isEmpty
                  ? 'No clinics available'
                  : 'No clinics found',
              style: TextStyle(fontSize: 18, color: context.subtextColor),
            ),
          ],
        ),
      );
    ...
    if (_filteredDoctors.isEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.person, size: 64, color: context.subtextColor),
            const SizedBox(height: 16),
            Text(
              _searchQuery.isEmpty
            ...
            children: [
                    ...
                    child: ClipRRect(
                      ...
                      child:
                          clinic['profilePhoto'] != null &&
                              clinic['profilePhoto'].toString().isNotEmpty
                          ? Image.network(
                              ...
                              loadingBuilder:
                                  (context, child, loadingProgress) {
                                    ...
                                    return Container(
                                      ...
                                      height: 50,
                                      decoration: BoxDecoration(
                                        color: colors().bluecolor1.withOpacity(
                                          0.1,
                                        ),
                                        borderRadius: BorderRadius.circular(25),
                                      ),
              ...
              Row(
                children: [
                  ...
                  FutureBuilder<bool>(
                    future: NotificationService().isClinicNotificationEnabled(
                      clinic['id'],
                    ),
                        ...
                        onTap: () async {
                          if (isEnabled) {
                            await NotificationService()
                                .removeClinicNotificationWithSync(clinic['id']);
                          } else {
                            await NotificationService()
  ...
  Widget _buildDoctorCard(Map<String, dynamic> doctor) {
    ...
    final hasAvailableClinic = clinics.any(
      (c) => c['isOpen'] == true && c['isAvailable'] == true,
    );
                    ...
                    child: ClipRRect(
                      ...
                      child:
                          doctor['profilePhoto'] != null &&
                              doctor['profilePhoto'].toString().isNotEmpty
                          ? Image.network(
                              ...
                              child:
                                  clinic['profilePhoto'] != null &&
                                      clinic['profilePhoto']
                                          ...
                                          .isNotEmpty
                                  ? Image.network(
                                      clinic['profilePhoto'].startsWith(
                                            'https://',
                                          )
                                          ? clinic['profilePhoto']
                                          : '${ApiConfig.currentBaseUrl.replaceAll('/api', '')}${clinic['profilePhoto']}',
