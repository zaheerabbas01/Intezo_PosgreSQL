...
class ClinicProvider with ChangeNotifier {
  ...
  Future<void> selectClinic(Map<String, dynamic> clinic) async {
    ...
    await loadClinicStatus(clinic['id']);
    await loadCurrentQueue(clinic['id']);
  }
  ...
  Future<void> loadClinicStatus(String clinicId) async {
    try {
      ...
      if (_selectedClinic != null && _selectedClinic!['id'] == clinicId) {
  ...
  Future<void> loadCurrentQueue(
    ...
    try {
      final response = await ClinicService.getRealTimeQueue(
        clinicId,
        doctorId: doctorId,
      );
  ...
  Future<Map<String, dynamic>> bookQueueNumber(
    String clinicId,
    String patientId, {
    String? doctorId,
    String? patientName,
  ...
  Future<Map<String, dynamic>?> getPatientCurrentQueue() async {
    ...
    try {
      ...
      // Backend returns 404 with { error: '...' } when no active queue
      if (response.containsKey('error')) return null;

      // Patient was already served
      ...
      _isLoading = false;
      notifyListeners();
      // 404 = no active booking, not a real error
      if (e.toString().contains('404') ||
          e.toString().contains('No active queue')) {
        return null;
      }
      _error = e.toString();
      notifyListeners();
  ...
  void startListeningForUpdates(String clinicId, {String? doctorId}) {
    ...
    if (_currentListeningClinicId == clinicId &&
      ...
      print(
        'Already listening for updates on clinic $clinicId, doctor: $doctorId',
      );
      return;
    }
    ...
    _queueUpdateSubscription = EventBus().onQueueUpdate.listen((event) {
      ...
      final isRelevantUpdate =
          event.clinicId == clinicId &&
          (event.doctorId == doctorId || event.doctorId == null);

      if (isRelevantUpdate) {
        print(
          'Real-time queue update received for clinic $clinicId, doctor: $doctorId: ${event.queueData}',
        );

        _currentQueue = {
          'current':
              event.queueData['currentNumber'] ??
              event.queueData['current'] ??
              0,
          'nextNumber':
              (event.queueData['currentNumber'] ??
                  event.queueData['current'] ??
    ...
    _clinicStatusSubscription = EventBus().onClinicStatusUpdate.listen((event) {
      ...
      final index = _clinics.indexWhere((c) => c['id'] == event.clinicId);
      if (index != -1) {
        _clinics[index]['isOpen'] = event.statusData['isOpen'];
        _clinics[index]['lastStatusChange'] =
            event.statusData['lastStatusChange'];
        notifyListeners();
      }

      // Update selected clinic if it's the same
      if (_selectedClinic != null && _selectedClinic!['id'] == event.clinicId) {
        _selectedClinic = {
          ..._selectedClinic!,
    ...
    _doctorAvailabilitySubscription = EventBus().onDoctorAvailabilityUpdate.listen((
      event,
    ) {
      if (event.clinicId == clinicId) {
        print(
          'Real-time doctor availability update received: ${event.doctorName} - ${event.isAvailable}',
        );
        notifyListeners();
      }
    });
  ...
  Future<void> _connectToSocket(String clinicId, {String? doctorId}) async {
    try {
      ...
      if (!SocketService.instance.isConnected) {
        ...
        await SocketService.instance.joinClinicRoom(
          clinicId,
          doctorId: doctorId,
        );
      }
      print(
        'Socket.IO connection established for clinic: $clinicId${doctorId != null ? ', doctor: $doctorId' : ''}',
      );
    } catch (e) {
