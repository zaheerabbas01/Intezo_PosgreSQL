...
class _Hospital_SuggestionState extends State<Hospital_Suggestion> {
  ...
  Future<void> _loadPreviouslyBookedClinics() async {
    try {
      ...
      final cachedClinics = await PreloadService.getCachedRecentClinics(patientId);
      
      if (cachedClinics.isNotEmpty) {
        _previouslyBookedClinics = widget.clinics.where((clinic) {
          return cachedClinics.any((c) => c['_id'] == clinic['_id']);
        }).toList();
        
        // Sort by cached order (most recent first)
        _previouslyBookedClinics.sort((a, b) {
          final aIndex = cachedClinics.indexWhere((c) => c['_id'] == a['_id']);
          final bIndex = cachedClinics.indexWhere((c) => c['_id'] == b['_id']);
                    ...
                    leading: Container(
                      ...
                      child: ClipRRect(
                        ...
                        child: updatedClinic['profilePhoto'] != null
                            ? Image.network(
                                updatedClinic['profilePhoto'].startsWith('https://') 
                                  ...
                                  return GestureDetector(
                                    onTap: () async {
                                      if (isEnabled) {
                                        ...
                                        await NotificationService().addClinicNotificationWithSync(clinic['_id'], clinicName);
                                      }
                                      setState(() {});
                                    },
                                    child: Container(
                                      ...
                                      decoration: BoxDecoration(
                                        color: isEnabled ? Colors.orange : context.primaryColor.withOpacity(0.1),
                                        borderRadius: BorderRadius.circular(6),
                                      ),
                                      child: Icon(
                                        isEnabled ? Icons.notifications_off : Icons.notifications,
                                        size: 14,
                                        color: isEnabled ? Colors.white : context.primaryColor,
                                      ),
                                    ),
                                  );
                                },
