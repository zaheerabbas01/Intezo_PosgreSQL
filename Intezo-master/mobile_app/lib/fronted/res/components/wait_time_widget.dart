...
class _WaitTimeWidgetState extends State<WaitTimeWidget> {
  ...
  Future<void> _loadWaitTime() async {
    try {
      ...
      final waitTime = await WaitTimeService.getWaitTime(widget.clinicId, widget.doctorId);
  ...
  Widget _buildSimpleView() {
    ...
    final estimatedTime = WaitTimeService.formatWaitTime(totalWaiting * avgProcessTime);
  ...
  Widget _buildDetailedView() {
    ...
    final totalWaiting = _waitTimeData!['totalWaiting'] ?? 0;
    final avgProcessTime = _waitTimeData!['avgProcessTimeMinutes'] ?? 15;
    final totalEstimatedTime = _waitTimeData!['totalEstimatedTime'] ?? 'No wait';
