...
const PremiumPayments = () => {
  ...
  const fetchPendingPayments = async () => {
    try {
      ...
      const response = await fetch('http://localhost:3000/api/admin/premium-payments', {
        headers: {
  ...
  const handleApprove = async (paymentId) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`http://localhost:3000/api/admin/premium-payments/${paymentId}/approve`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleReject = async () => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`http://localhost:3000/api/admin/premium-payments/${selectedPayment._id}/reject`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
