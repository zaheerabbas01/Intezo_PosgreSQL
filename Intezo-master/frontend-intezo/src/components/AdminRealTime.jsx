...
const AdminRealTime = () => {
  ...
  useEffect(() => {
    ...
    const adminSocket = io(`${API_CONFIG.socketUrl}/admin`, {
      auth: { token },
  ...
  const fetchActivities = async () => {
    try {
      const response = await fetch(`${API_CONFIG.baseUrl}/admin/activity`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('adminToken')}` }
      });
      const data = await response.json();
  ...
  const fetchOnlineUsers = async () => {
    try {
      const response = await fetch(`${API_CONFIG.baseUrl}/admin/online-users`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('adminToken')}` }
      });
      const data = await response.json();
