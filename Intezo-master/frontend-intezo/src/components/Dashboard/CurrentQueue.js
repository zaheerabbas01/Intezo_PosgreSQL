...
const CurrentQueue = ({ currentNumber, currentQueueId, onNext, onSkip, canCallNext, totalWaiting, nextPatientNumber, isFinishingCurrent }) => {
  return (
    <div className="current-queue">
      ...
      <div className="queue-actions">
        ...
        >
          {canCallNext ? (
            <>
              <span>{isFinishingCurrent ? 'Finish Current Patient' : 'Call Next Patient'}</span>
        ...
        {currentNumber > 0 && canCallNext && (
          <button
            onClick={onSkip}
            className="skip-patient-btn"
            title={`Skip current patient #${currentNumber}`}
          >
            <span>Skip Patient #{currentNumber}</span>
