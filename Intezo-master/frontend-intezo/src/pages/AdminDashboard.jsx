...
const AdminDashboard = () => {
  ...
  const handleSave = async () => {
    try {
      const response = await fetch(`${API_BASE}/admin/${modalType}s/${editForm.id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleRemoveClinic = async (doctorId, clinicId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const updatedClinics = editForm.clinics.filter(c => c.clinic.id !== clinicId);
  ...
  const handleRemoveDoctor = async (doctorId, clinicId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const updatedDoctors = editForm.doctors.filter(d => d.id !== doctorId);
            ...
            <tbody>
              {patients?.map(patient => (
                <tr key={patient.id}>
                  <td>
                    {patient.name}
                    <span className={`status ${isUserOnline(patient.id) ? 'online' : 'offline'}`}>
                      {isUserOnline(patient.id) ? '🟢' : '🔴'}
                    </span>
                  </td>
                  <td>{patient.email}</td>
                  <td>{patient.phone}</td>
            ...
            <tbody>
              {doctors?.map(doctor => (
                <tr key={doctor.id}>
                  <td>
                    {doctor.name}
                    <span className={`status ${isUserOnline(doctor.id) ? 'online' : 'offline'}`}>
                      {isUserOnline(doctor.id) ? '🟢' : '🔴'}
                    </span>
                  </td>
                  <td>{doctor.email}</td>
            ...
            <tbody>
              {clinics?.map(clinic => (
                <tr key={clinic.id}>
                  <td>
                    {clinic.name}
                    <span className={`status ${isUserOnline(clinic.id) ? 'online' : 'offline'}`}>
                      {isUserOnline(clinic.id) ? '🟢' : '🔴'}
                    </span>
                  </td>
                  ...
                  <td>
                    ...
                    <button 
                      className="delete-btn"
                      onClick={() => handleDelete('clinic', clinic.id)}
                    >
                      Delete
                    </button>
                  </td>
                  ...
                  {editForm.clinics?.map((clinicAssoc, index) => (
                    <div key={index} className="clinic-item">
                      ...
                      <button 
                        ...
                        onClick={() => handleRemoveClinic(editForm.id, clinicAssoc.clinic.id)}
                      >
                        Remove
                      </button>
                    </div>
                  ...
                  {editForm.doctors?.map((doctor, index) => (
                    <div key={index} className="clinic-item">
                      ...
                      <button 
                        type="button"
                        className="remove-clinic-btn"
                        onClick={() => handleRemoveDoctor(doctor.id, editForm.id)}
                      >
                        Remove
                      </button>
                    </div>
