...
const Patients = () => {
    ...
    const fetchQueueAnalytics = useCallback(async () => {
        try {
            console.log('Fetching queue analytics, currentUser:', currentUser);
            const { data } = await getQueueAnalytics();
            console.log('Queue analytics data:', data);
    ...
    const handleViewHistory = async (patient) => {
        ...
        try {
            ...
            if (patient.patient && patient.patient.id) {
                ...
                patientIdentifier = patient.patient.id;
            } else if (patient.id && patient.patient) {
    ...
    const handleMarkAsServed = async (patient) => {
        try {
            ...
            // await markPatientAsServed(patient.id);
    ...
    const handleCancelPatient = async (patient) => {
        ...
        try {
            // You'll need to create this API endpoint
            // await cancelPatient(patient.id);
    ...
    const handleSelectAll = (isSelected) => {
        if (isSelected) {
            setBulkSelection(getFilteredAndSortedPatients.map(p => p.id));
        } else {
            setBulkSelection([]);
        }
    };
    ...
    const getFilteredAndSortedPatients = useMemo(() => {
        let patients;
        switch (activeTab) {
            case 'waiting': patients = upcomingPatients; break;
            case 'served': patients = servedPatients; break;
            case 'cancelled': patients = cancelledPatients; break;
            default: patients = upcomingPatients;
        }
        ...
        return patients;
    }, [upcomingPatients, servedPatients, cancelledPatients, activeTab, searchTerm, filterBy, sortBy, sortOrder]);
                                ...
                                {patientHistory.length > 0 ? (
                                    <div className="history-list">
                                        {patientHistory.map((visit, index) => (
                                            <div key={visit.id || index} className="history-item">
                    ...
                    <div className="patients-card">
                        ...
                        <div className="patients-list-compact">
                            {getFilteredAndSortedPatients.map((patient, index) => (
                                <div 
                                    key={patient.id} 
                                    className={`patient-row ${bulkSelection.includes(patient.id) ? 'selected' : ''}`}
                                    ...
                                    {activeTab === 'waiting' && (
                                        <div className="patient-checkbox" onClick={(e) => e.stopPropagation()}>
                                            <input
                                                ...
                                                onChange={(e) => handleBulkSelection(patient.id, e.target.checked)}
                                            />
                                        </div>
                                    )}
